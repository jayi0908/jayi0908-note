...
通过 PlayCover 安装的应用，其主程序文件存放于 `~/Library/Containers/io.playcover.PlayCover/Applications/` 目录下的 `.app` 文件夹中，而游戏核心资源文件则存储在 `~/Library/Containers/` 目录下对应的应用容器目录内. PlayCover 下载的绝区零的特点是应用主体（.app）体积较小，但资源文件极其庞大，因此实现外接硬盘运行的核心思路很明晰：定位游戏容器目录，将其中的大体积资源文件迁移至外接硬盘，并通过挂载方式让游戏运行时能正确访问外接硬盘的资源文件.
